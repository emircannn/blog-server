...
exports.create= async (req, res)=>{
    try {
        const {name, text, note, user} = req.body
        const seo = utils.helpers.convertToSEOText(name)

        const findedText = await prisma.actual.findUnique({where: {title: name}})
        if(findedText) {
        ...
        const json = await prisma.actual.create({
            data: {
                image: textImage,
                seo,
                text,
...
exports.delete = async (req, res) => {
    try {
        const {seo} = req.query
        const findedText = await prisma.actual.findUnique({where: {seo}})
        const isDeleted = utils.helpers.deleteFromDisk(findedText.image ? findedText.image.split('uploads/')[1] : '')

        if(isDeleted) {
            return await prisma.actual.delete({where: {seo}})
        }
...
exports.update= async (req, res)=>{
    try {
        ...
        const findedText = name ? await prisma.actual.findUnique({where: {title: name}}) : undefined
        const _findedText = await prisma.actual.findUnique({where: {seo}})
        if(findedText) {
        ...
        if (isDeleted) {
            const json = await prisma.actual.update({
                where: {
                    seo
                },
                data: {
...
exports.getText = async (req, res) => {
    try {
        const {seo} = req.query
        const findedText = await prisma.actual.findUnique({where: {seo}, select: {
            id: true,
            userId: true,
            image: true,
            note: true,
